    ...
    def del_phone(self):    
        ...
        if len(self.phones) == 1:
            ...
            while ans != 'n' or ans != 'y':
                ans = input(f"Contact {self.name.value} has only 1 phone {self.phone.value}. Are you sure? (Y/N)").lower()    
            if ans == 'y':
                self.phones.clear()
        if len(self.phones) > 1:
            ...
            for i, phone in enumerate(self.phones,1):
                print(f'{i}: {phone}')
            num = input('Which one do you want to delete? (enter number):')
            self.phones.pop(num)
       
    def edit_phone(self):    
        if len(self.phones) == 0:
            print('this contact has no phones.')
            numb = input('You can add one:')
            phone(Phone) = input('You can add one:')
            
            self.phones.append(phone)
        if len(self.phones) == 1:
            print(f'Current phone number is {self.phone.value}')
            self.phone.value = input('Re-enter the phone number')
            
        if len(self.phones) > 1:
            print('this contact has several phones:')
            for i, phone in enumerate(self.phones,1):
                print(f'{i}: {phone}')
            num = input('Which one do you want to change? (enter number):')
            self.phones.pop(num)
