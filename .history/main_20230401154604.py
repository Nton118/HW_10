...
class Record:
    ...
    def merge_record(self, phone_new:Phone):
...
class AddressBook(UserDict):
    ...
    def add_record(self,record:Record):
        if record.name.value not in self.data.keys():
            ...
            record.merge_record(record.phones[0])
            self.data.update({record.name.value:record})
...
#             if func.__name__ == 'change' or func.__name__ == 'phone' or func.__name__ == 'del_contact':
...
def add(contact:str, phone:str = None):
    ...
    if contact not in book1.keys(): 
        ...
        book1.add_record(rec_new)
        return f'Updated existing contact "{contact}" with phone number: {phone}'
...
def change(contact:str, phone:str = None):    
    ...
    if len(rec.phones) == 0: 
        ...
        return f'Changed phone number {old_phone} to {phone_new.value} for contact "{contact}"' 
