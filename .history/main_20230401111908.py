...
class Phone(Field):
    ...
    def __init__(self, value):
            ...
            if not new_phone.isdecimal() or not Phone.min_len <= len(new_phone) <= Phone.max_len:
                self.value = None
                raise ValueError
...
def del_contact(contact:str):
    ...
    ans = None
    while ans != 'n' or ans != 'y':
        ans = input(f"Are you sure to delete contact {rec.name.value}? (Y/N)").lower()
    if ans == 'y':
